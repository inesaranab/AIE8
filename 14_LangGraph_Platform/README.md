...
##### ✅ Answer:
`agent_helpful` goes one step further than `agent` and introduces `helpfulness_node`. In this node, an LLM judges whether the agent's final response is helpful towards the initial query, appending a **HELPFULNESS:Y**/**HELPFULNESS:N** message with the Assistant's response/decision to the state. It guards against infinite cycles by automatically appending an AIMessage with **HELPFULNESS:END** if the length of the messages exceeds a threshold (>10). Execution routes back to the agent based on the `helpfulness_decision` conditional edge function, which examines the decision contained in the state: if helpful, we go to __end__; if not, we continue looping.
...
##### ✅ Answer:
We use a *Before* interrupt to debug the input of a node (for example, if the correct tool has been selected). On the other hand, we can use an *After* interrupt to debug the output of a node (for example, why the helpfulness node has decided towards helpful or not helpful).
